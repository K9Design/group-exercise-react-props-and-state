...
function App() {
  ...
  const [chuckImage, setChuckImage] = useState(
    "https://images-global.nhst.tech/image/R1dwYnJhOTc5RjNCTG5URi9BMUlIZGE5cTRtemFrUWFBSVNINUMvZ20xZz0=/nhst/binary/f1ffde963b9087457feb601be1842d16?image_version=1200"
  );
  ...
  let joke = jokes.filter(({ id }) => id === 3);
  console.log(joke)

  return (
    <div className="App">
      <h1>React props and state</h1>
      <ChuckNorrisCard chuckGreeting={chuckGreeting} chuckImage={chuckImage} />
      ...
      <h2>Jokes: </h2>
      {jokes.map(({ joke, id }) => (
        <ChuckJokes key={id} joke={joke} />
      ))}

      <h2>Filtered: </h2>
      {<ChuckJokes key={joke[0].id} joke={joke[0].joke} />}
    </div>
  );
}
